...
Firmware builds with a version suffix, such as `-sync`, do not check for or
download official releases automatically. This prevents a custom build from
being replaced by an official build that does not contain its features. USB
flashing and manual uploads through the web interface remain available.
